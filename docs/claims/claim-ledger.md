...
| 9 | “The company brief opens in English as the audited source of record. In the Traditional Chinese and Korean editions, structure and citations are checked automatically — section count and order, citation markers per section, and no numeric literal absent from the source — and the wording itself is not evaluated. Review and approval stay tied to the English original.” | README, `/brief`, diagram | `check_translation_shape` in `backend/app/briefs/translation.py`; shape tests in `backend/tests/test_translation.py`; `frontend/app/components/BriefCanvas.tsx`; route and browser tests | Supported limitation |
| 9a | “Radar news translations are not evaluated and carry no automatic checks of any kind. The brief’s shape checks do not run on them.” | Radar footer, onboarding, `docs/MARKET_RADAR.md` | `translate_news_items` in `backend/app/market_radar/service.py` — no call to `check_translation_shape`, which is reached only from `translate_brief_payload` | Supported limitation |
