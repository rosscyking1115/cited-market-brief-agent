...
function macroTiles(changes: ChangesPayload): MarketTile[] {
  const tiles = changes.macro_deltas.slice(0, 4).map((delta) => {
    ...
    return {
      label: delta.series_id,
      value: formatNumber(delta.latest_value),
      detail:
        change == null
          ? delta.latest_date ?? "latest"
  ...
  return [
    { label: "New filings", value: String(changes.new_documents.length), detail: "SEC EDGAR", tone: changes.new_documents.length ? "flag" : "flat" },
    { label: "Filing diffs", value: String(changes.filing_diffs.length), detail: "since last brief", tone: changes.filing_diffs.length ? "flag" : "flat" },
  ];
}
  ...
  return (
    <section className="overflow-hidden rounded-(--radius-card) border border-hairline bg-card">
      <div className="flex flex-col gap-2 border-b border-hairline px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="th-label">Morning context</p>
          <h2 className="reader-heading mt-1 text-[15px] font-semibold text-neutral-30">
            Market, macro, and filing signal check
          </h2>
        </div>
        <p className="reader-meta max-w-xl text-[11px] leading-relaxed text-neutral-90">
          Built for the first read: scan the live deltas, then open the cited proof before approving or exporting.
        </p>
      </div>
