...
def main() -> None:
    ...
    try:
        ...
        briefs_by_status = dict(
            db.execute(select(Brief.status, func.count(Brief.id)).group_by(Brief.status)).all()
        )
        claims_total = db.scalar(select(func.count(Claim.id))) or 0
        claims_supported = db.scalar(
            select(func.count(Claim.id)).where(Claim.support_status == SupportStatus.SUPPORTED)
        ) or 0
        ...
        wrong_rate = (
            100.0 * feedback_by_kind.get("wrong", 0) / claims_total if claims_total else 0.0
        )
        exports_by_fmt = dict(
            db.execute(select(Export.fmt, func.count(Export.id)).group_by(Export.fmt)).all()
        )
        run_failures = db.scalar(
            select(func.count(AuditEvent.id)).where(AuditEvent.action == "pilot.run_failed")
        ) or 0
        watchlists = db.scalar(select(func.count(Watchlist.id))) or 0
        approval_rate = (
            100.0 * briefs_by_status.get("approved", 0) / briefs_total if briefs_total else 0.0
        )
        ...
        lines = [
            ...
            f"| Feedback: wrong | {feedback_by_kind.get('wrong', 0)} ({wrong_rate:.1f}% of claims) "
            f"| trending to 0 |",
